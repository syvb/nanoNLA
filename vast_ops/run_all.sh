...
# persist the regenerated FULL parquets before the long sweep (background, ~1GB)
step "01c upload data (bg)"; ( bash "$HERE/upload_data.sh" >"$LOGS/01c_upload_data.log" 2>&1 || echo "data upload failed" ) &
